...
description: |
  ...
  assistant: "I'll use /cesdk:docs-svelte to look up configuration options."
  </example>

  <example>
  Context: User needs Svelte integration
  user: "How do I use CE.SDK in a Svelte component?"
  assistant: "Let me use /cesdk:docs-svelte to find the relevant documentation."
  </example>
...
1. **Index lookup**: Match the query to a path in the Documentation Index above.
   ...
   Resolve the path using Glob: `**/skills/docs-svelte/<path>.md`
   (e.g., for `text/add.md` → `**/skills/docs-svelte/text/add.md`)
2. **Search**: If no exact match exists, use Grep to search the documentation:
   `Grep pattern="<keyword>" path="**/skills/docs-svelte/" output_mode="content"`
   or search filenames: `Glob pattern="**/skills/docs-svelte/**/*<keyword>*.md"`
...
4. **Check rules** if the topic involves setup, initialization, or common
   operations: `**/skills/docs-svelte/rules/*.md`
...
1. **Module lookup**: Match the query to a module in the API index above.
   Read the module file: `**/skills/docs-svelte/api/<ModuleName>.md`
   (e.g., for BlockAPI → `**/skills/docs-svelte/api/BlockAPI.md`)
2. **Method search**: If looking for a specific method, use Grep:
   `Grep pattern="<method>" path="**/skills/docs-svelte/api/" output_mode="content"`
3. **For common types**: Read `**/skills/docs-svelte/api/types.md`

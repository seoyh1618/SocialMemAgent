...
4. Start generating ideas to maximize user engagment. Follow the following steps:
    ...
    (5). Generate a list of hashtags (hashtags) — 8-15 relevant hashtags for social media posts. Include a mix of:
    ...
    (6). Output every thing into a json format, the key should be `text_prompt`, `image_prompt`, `video_prompt`, `narration_prompt`, `hashtags`. In the image_prompt, include each aspect as an key and the following descriptions as the value.

**IMPORTANT**
- Your output must be in json format, the key should be `text_prompt`, `image_prompt`, `video_prompt`, `narration_prompt`, `hashtags`.

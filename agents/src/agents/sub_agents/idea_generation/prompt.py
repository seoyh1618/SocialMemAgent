...
4. Start generating ideas to maximize user engagment. Follow the following steps:
    ...
    (3). Third, generate 1-2 sentence of text (text_prompt) as a title or short comment for the post.
    (4). Generate a list of hashtags (hashtags) — 8-15 relevant hashtags for social media posts. Include a mix of:
         - Brand-specific tags (if known from context)
         - Content-relevant tags based on the campaign theme
         - Trending/popular tags in the domain
    (5). Output everything into a json format, the key should be `text_prompt`, `image_prompt`, `hashtags`. In the image_prompt, include each aspect as a key and the following descriptions as the value.

**IMPORTANT**
- Your output must be in json format, the key should be `text_prompt`, `image_prompt`, `hashtags`.
- `text_prompt` and `hashtags` MUST be in the SAME LANGUAGE as the user's query. If the user writes in Korean, these must be in Korean.
- If you don't have enough information to generate the idea, you can leave the value as an empty string.
"""
...
export default function ImagePromptBlock({ base, setBase }: ImagePromptBlockProps) {
    return (
        <BaseBlock
            icon={PhotoIcon}
            title="Image Prompt"
                    ...
                    <div className="mt-2">
                        ...
                        >
                            {base.image_prompt.value === "" ? (
                                    ...
                                    <textarea
                                        rows={9}
                                        name="image_prompt_input"
                                        id="image_prompt_input"
                                        value={base.image_prompt.value}
                                        className="block w-full rounded-md border-0 px-2 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                                        onChange={(e) => {
                                            const updatedBase = { ...base, image_prompt: { value: e.target.value, enabled: true } };
                                            setBase(updatedBase);
                                        }}
                                    />
                                )}
